...
public class CodeSearchJfc {
    private static Logger LOG = Logger.getLogger(CodeSearchJfc.class.getName());
    
    private CodeSearchModel model;
    private JDialog dialog;
    protected CodeJfc jfcCode;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected Code codeSelected;
    protected boolean bSelected;
    
    public CodeSearchJfc() {
        this(null, true, false);
    }
    public CodeSearchJfc(CodeSearchModel model) {
        this(model, true, false);
    }
    public CodeSearchJfc(CodeSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public CodeSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public CodeSearchModel getModel() {
        if (model == null) {
            model = new CodeSearchModel();
            model.getCodeSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("code.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<Code> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Code code = hub.getAO();
                    if (code != null) onSelect(code, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<Code> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("Codes to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for Code
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Code code = model.getHub().getAO(); 
                    if (code != null) onSelect(code, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                CodeJfc jfcCode = new CodeJfc(model);
                OATree tree = jfcCode.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getCodeSearchHub(), CodeSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<Code> hubCode) {
        Hub<Code> hubx = new Hub(Code.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (CodeSearchJfc.this.getModel().getMultiSelectHub() != hubCode) {
                    Code code = hubCode.getAO();
                    if (code != null) onSelect(code, hubCode);
                }
            }
        };
        table.setFilterMasterHub(hubCode);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubCode) {
    ...
    protected void createTableColumns(OATable table) {
        getCodeJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<Code> hub) {
        ...
        cmd.setToolTipText("Add selected Code");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected Codes");
    ...
    public OAButton createRemoveButton(Hub<Code> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected Code");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<Code> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all Codes from the select list");
        return cmd;
    }
    ...
    public CodeJfc getCodeJfc() {
        if (jfcCode == null) {
            jfcCode = new CodeJfc(new Hub<Code>(Code.class));
            OAObjectModel om = jfcCode.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<Code> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getCodeJfc().getHub().setSharedHub(hub, true);
                getCodeJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Code Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                CodeSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!CodeSearchJfc.this.bSelected) {
                    CodeSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getCodeSearch().reset();
    }
    
    protected void onSelect(Code code, Hub<Code> hubFrom) {
        codeSelected = code;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(code, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<Code> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getCodeSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "Code Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting Codes ... please wait ...");
        dlgWait.setTitle("Code Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No Codes were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            int max = getModel().getCodeSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Codes.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "Code Search",
