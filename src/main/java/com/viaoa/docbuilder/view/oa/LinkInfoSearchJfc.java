...
public class LinkInfoSearchJfc {
    private static Logger LOG = Logger.getLogger(LinkInfoSearchJfc.class.getName());
    
    private LinkInfoSearchModel model;
    private JDialog dialog;
    protected LinkInfoJfc jfcLinkInfo;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected LinkInfo linkInfoSelected;
    protected boolean bSelected;
    
    public LinkInfoSearchJfc() {
        this(null, true, false);
    }
    public LinkInfoSearchJfc(LinkInfoSearchModel model) {
        this(model, true, false);
    }
    public LinkInfoSearchJfc(LinkInfoSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public LinkInfoSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public LinkInfoSearchModel getModel() {
        if (model == null) {
            model = new LinkInfoSearchModel();
            model.getLinkInfoSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("linkInfo.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<LinkInfo> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    LinkInfo linkInfo = hub.getAO();
                    if (linkInfo != null) onSelect(linkInfo, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<LinkInfo> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("LinkInfos to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for LinkInfo
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final LinkInfoModel model = new LinkInfoModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    LinkInfo linkInfo = model.getHub().getAO(); 
                    if (linkInfo != null) onSelect(linkInfo, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                LinkInfoJfc jfcLinkInfo = new LinkInfoJfc(model);
                OATree tree = jfcLinkInfo.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getLinkInfoSearchHub(), LinkInfoSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<LinkInfo> hubLinkInfo) {
        Hub<LinkInfo> hubx = new Hub(LinkInfo.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (LinkInfoSearchJfc.this.getModel().getMultiSelectHub() != hubLinkInfo) {
                    LinkInfo linkInfo = hubLinkInfo.getAO();
                    if (linkInfo != null) onSelect(linkInfo, hubLinkInfo);
                }
            }
        };
        table.setFilterMasterHub(hubLinkInfo);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubLinkInfo) {
    ...
    protected void createTableColumns(OATable table) {
        getLinkInfoJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<LinkInfo> hub) {
        ...
        cmd.setToolTipText("Add selected LinkInfo");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected LinkInfos");
    ...
    public OAButton createRemoveButton(Hub<LinkInfo> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected LinkInfo");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<LinkInfo> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all LinkInfos from the select list");
        return cmd;
    }
    ...
    public LinkInfoJfc getLinkInfoJfc() {
        if (jfcLinkInfo == null) {
            jfcLinkInfo = new LinkInfoJfc(new Hub<LinkInfo>(LinkInfo.class));
            OAObjectModel om = jfcLinkInfo.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<LinkInfo> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getLinkInfoJfc().getHub().setSharedHub(hub, true);
                getLinkInfoJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Link Info Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                LinkInfoSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!LinkInfoSearchJfc.this.bSelected) {
                    LinkInfoSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getLinkInfoSearch().reset();
    }
    
    protected void onSelect(LinkInfo linkInfo, Hub<LinkInfo> hubFrom) {
        linkInfoSelected = linkInfo;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(linkInfo, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<LinkInfo> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getLinkInfoSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "LinkInfo Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting LinkInfos ... please wait ...");
        dlgWait.setTitle("LinkInfo Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No LinkInfos were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            int max = getModel().getLinkInfoSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Link Infos.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "LinkInfo Search",
    ...
    public LinkInfo getSelected() {
        return linkInfoSelected;
    }
