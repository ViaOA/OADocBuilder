...
public class ProjectInfoSearchJfc {
    private static Logger LOG = Logger.getLogger(ProjectInfoSearchJfc.class.getName());
    
    private ProjectInfoSearchModel model;
    private JDialog dialog;
    protected ProjectInfoJfc jfcProjectInfo;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected ProjectInfo projectInfoSelected;
    protected boolean bSelected;
    
    public ProjectInfoSearchJfc() {
        this(null, true, false);
    }
    public ProjectInfoSearchJfc(ProjectInfoSearchModel model) {
        this(model, true, false);
    }
    public ProjectInfoSearchJfc(ProjectInfoSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public ProjectInfoSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public ProjectInfoSearchModel getModel() {
        if (model == null) {
            model = new ProjectInfoSearchModel();
            model.getProjectInfoSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("projectInfo.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<ProjectInfo> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ProjectInfo projectInfo = hub.getAO();
                    if (projectInfo != null) onSelect(projectInfo, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<ProjectInfo> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("ProjectInfos to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for ProjectInfo
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final ProjectInfoModel model = new ProjectInfoModel(ModelDelegate.getProjectInfos().createSharedHub());
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ProjectInfo projectInfo = model.getHub().getAO(); 
                    if (projectInfo != null) onSelect(projectInfo, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                ProjectInfoJfc jfcProjectInfo = new ProjectInfoJfc(model);
                OATree tree = jfcProjectInfo.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<ProjectInfo> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createIdTextField() {
        OATextField txt = new OATextField(getModel().getProjectInfoSearchHub(), ProjectInfoSearch.P_Id, 6);
    ...
    public OATextField createId2TextField() {
        OATextField txt = new OATextField(getModel().getProjectInfoSearchHub(), ProjectInfoSearch.P_Id2, 6);
        txt.setMinimumColumns(0);
        txt.setMaximumColumns(8);
        // setup(txt);
        return txt;
    ...
    public OADateComboBox createCreatedDateComboBox() {
        OADateComboBox dcbo = new OADateComboBox(getModel().getProjectInfoSearchHub(), ProjectInfoSearch.P_Created, 15);
        dcbo.setMaximumColumns(22);
        OATextField txt = new OATextField(getModel().getProjectInfoSearchHub(), ProjectInfoSearch.P_Created, 10);
        dcbo.setEditor(txt);
        return dcbo;
    }
    
    public OATextField createNameTextField() {
        OATextField txt = new OATextField(getModel().getProjectInfoSearchHub(), ProjectInfoSearch.P_Name, 20);
        txt.setMinimumColumns(0);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getProjectInfoSearchHub(), ProjectInfoSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<ProjectInfo> hubProjectInfo) {
        Hub<ProjectInfo> hubx = new Hub(ProjectInfo.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (ProjectInfoSearchJfc.this.getModel().getMultiSelectHub() != hubProjectInfo) {
                    ProjectInfo projectInfo = hubProjectInfo.getAO();
                    if (projectInfo != null) onSelect(projectInfo, hubProjectInfo);
                }
            }
        };
        table.setFilterMasterHub(hubProjectInfo);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubProjectInfo) {
    ...
    protected void createTableColumns(OATable table) {
        getProjectInfoJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<ProjectInfo> hub) {
        ...
        cmd.setToolTipText("Add selected ProjectInfo");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected ProjectInfos");
    ...
    public OAButton createRemoveButton(Hub<ProjectInfo> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected ProjectInfo");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<ProjectInfo> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all ProjectInfos from the select list");
        return cmd;
    }
    ...
    public ProjectInfoJfc getProjectInfoJfc() {
        if (jfcProjectInfo == null) {
            jfcProjectInfo = new ProjectInfoJfc(new Hub<ProjectInfo>(ProjectInfo.class));
            OAObjectModel om = jfcProjectInfo.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<ProjectInfo> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getProjectInfoJfc().getHub().setSharedHub(hub, true);
                getProjectInfoJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Project Info Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ProjectInfoSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!ProjectInfoSearchJfc.this.bSelected) {
                    ProjectInfoSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getProjectInfoSearch().reset();
    }
    
    protected void onSelect(ProjectInfo projectInfo, Hub<ProjectInfo> hubFrom) {
        projectInfoSelected = projectInfo;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(projectInfo, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<ProjectInfo> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getProjectInfoSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "ProjectInfo Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting ProjectInfos ... please wait ...");
        dlgWait.setTitle("ProjectInfo Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No ProjectInfos were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "ProjectInfo Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getProjectInfoSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Project Infos.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "ProjectInfo Search",
    ...
    public ProjectInfo getSelected() {
        return projectInfoSelected;
    }
