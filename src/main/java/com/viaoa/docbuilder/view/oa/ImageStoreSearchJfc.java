...
public class ImageStoreSearchJfc {
    private static Logger LOG = Logger.getLogger(ImageStoreSearchJfc.class.getName());
    
    private ImageStoreSearchModel model;
    private JDialog dialog;
    protected ImageStoreJfc jfcImageStore;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected ImageStore imageStoreSelected;
    protected boolean bSelected;
    
    public ImageStoreSearchJfc() {
        this(null, true, false);
    }
    public ImageStoreSearchJfc(ImageStoreSearchModel model) {
        this(model, true, false);
    }
    public ImageStoreSearchJfc(ImageStoreSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public ImageStoreSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public ImageStoreSearchModel getModel() {
        if (model == null) {
            model = new ImageStoreSearchModel();
            model.getImageStoreSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("imageStore.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<ImageStore> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ImageStore imageStore = hub.getAO();
                    if (imageStore != null) onSelect(imageStore, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<ImageStore> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("ImageStores to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for ImageStore
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final ImageStoreModel model = new ImageStoreModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ImageStore imageStore = model.getHub().getAO(); 
                    if (imageStore != null) onSelect(imageStore, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                ImageStoreJfc jfcImageStore = new ImageStoreJfc(model);
                OATree tree = jfcImageStore.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<ImageStore> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getImageStoreSearchHub(), ImageStoreSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<ImageStore> hubImageStore) {
        Hub<ImageStore> hubx = new Hub(ImageStore.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (ImageStoreSearchJfc.this.getModel().getMultiSelectHub() != hubImageStore) {
                    ImageStore imageStore = hubImageStore.getAO();
                    if (imageStore != null) onSelect(imageStore, hubImageStore);
                }
            }
        };
        table.setFilterMasterHub(hubImageStore);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubImageStore) {
    ...
    protected void createTableColumns(OATable table) {
        getImageStoreJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<ImageStore> hub) {
        ...
        cmd.setToolTipText("Add selected ImageStore");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected ImageStores");
    ...
    public OAButton createRemoveButton(Hub<ImageStore> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected ImageStore");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<ImageStore> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all ImageStores from the select list");
        return cmd;
    }
    ...
    public ImageStoreJfc getImageStoreJfc() {
        if (jfcImageStore == null) {
            jfcImageStore = new ImageStoreJfc(new Hub<ImageStore>(ImageStore.class));
            OAObjectModel om = jfcImageStore.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<ImageStore> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getImageStoreJfc().getHub().setSharedHub(hub, true);
                getImageStoreJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Image Store Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ImageStoreSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!ImageStoreSearchJfc.this.bSelected) {
                    ImageStoreSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getImageStoreSearch().reset();
    }
    
    protected void onSelect(ImageStore imageStore, Hub<ImageStore> hubFrom) {
        imageStoreSelected = imageStore;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(imageStore, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<ImageStore> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getImageStoreSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "ImageStore Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting ImageStores ... please wait ...");
        dlgWait.setTitle("ImageStore Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No ImageStores were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "ImageStore Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getImageStoreSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Image Stores.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "ImageStore Search",
    ...
    public ImageStore getSelected() {
        return imageStoreSelected;
    }
