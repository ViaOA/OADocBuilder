...
public class TutorialSearchJfc {
    private static Logger LOG = Logger.getLogger(TutorialSearchJfc.class.getName());
    
    private TutorialSearchModel model;
    private JDialog dialog;
    protected TutorialJfc jfcTutorial;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected Tutorial tutorialSelected;
    protected boolean bSelected;
    
    public TutorialSearchJfc() {
        this(null, true, false);
    }
    public TutorialSearchJfc(TutorialSearchModel model) {
        this(model, true, false);
    }
    public TutorialSearchJfc(TutorialSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public TutorialSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public TutorialSearchModel getModel() {
        if (model == null) {
            model = new TutorialSearchModel();
            model.getTutorialSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("tutorial.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<Tutorial> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Tutorial tutorial = hub.getAO();
                    if (tutorial != null) onSelect(tutorial, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<Tutorial> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("Tutorials to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for Tutorial
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final TutorialModel model = new TutorialModel(ModelDelegate.getTutorials().createSharedHub());
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Tutorial tutorial = model.getHub().getAO(); 
                    if (tutorial != null) onSelect(tutorial, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                TutorialJfc jfcTutorial = new TutorialJfc(model);
                OATree tree = jfcTutorial.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getTutorialSearchHub(), TutorialSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<Tutorial> hubTutorial) {
        Hub<Tutorial> hubx = new Hub(Tutorial.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (TutorialSearchJfc.this.getModel().getMultiSelectHub() != hubTutorial) {
                    Tutorial tutorial = hubTutorial.getAO();
                    if (tutorial != null) onSelect(tutorial, hubTutorial);
                }
            }
        };
        table.setFilterMasterHub(hubTutorial);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubTutorial) {
    ...
    protected void createTableColumns(OATable table) {
        getTutorialJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<Tutorial> hub) {
        ...
        cmd.setToolTipText("Add selected Tutorial");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected Tutorials");
    ...
    public OAButton createRemoveButton(Hub<Tutorial> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected Tutorial");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<Tutorial> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all Tutorials from the select list");
        return cmd;
    }
    ...
    public TutorialJfc getTutorialJfc() {
        if (jfcTutorial == null) {
            jfcTutorial = new TutorialJfc(new Hub<Tutorial>(Tutorial.class));
            OAObjectModel om = jfcTutorial.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<Tutorial> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getTutorialJfc().getHub().setSharedHub(hub, true);
                getTutorialJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Tutorial Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                TutorialSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!TutorialSearchJfc.this.bSelected) {
                    TutorialSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getTutorialSearch().reset();
    }
    
    protected void onSelect(Tutorial tutorial, Hub<Tutorial> hubFrom) {
        tutorialSelected = tutorial;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(tutorial, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<Tutorial> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getTutorialSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "Tutorial Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting Tutorials ... please wait ...");
        dlgWait.setTitle("Tutorial Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No Tutorials were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            int max = getModel().getTutorialSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Tutorials.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "Tutorial Search",
