...
public class SectionSearchJfc {
    private static Logger LOG = Logger.getLogger(SectionSearchJfc.class.getName());
    
    private SectionSearchModel model;
    ...
    public SectionSearchJfc() {
        this(null, true, false);
    }
    public SectionSearchJfc(SectionSearchModel model) {
        this(model, true, false);
    }
    public SectionSearchJfc(SectionSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public SectionSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public SectionSearchModel getModel() {
        if (model == null) {
            model = new SectionSearchModel();
            model.getSectionSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("section.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<Section> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Section section = hub.getAO();
                    if (section != null) onSelect(section, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<Section> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("Sections to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for Section
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final SectionModel model = new SectionModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Section section = model.getHub().getAO(); 
                    if (section != null) onSelect(section, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                SectionJfc jfcSection = new SectionJfc(model);
                OATree tree = jfcSection.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getSectionSearchHub(), SectionSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<Section> hubSection) {
        Hub<Section> hubx = new Hub(Section.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (SectionSearchJfc.this.getModel().getMultiSelectHub() != hubSection) {
                    Section section = hubSection.getAO();
                    if (section != null) onSelect(section, hubSection);
                }
            }
        };
        table.setFilterMasterHub(hubSection);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubSection) {
    ...
    protected void createTableColumns(OATable table) {
        getSectionJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<Section> hub) {
        ...
        cmd.setToolTipText("Add selected Section");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected Sections");
    ...
    public OAButton createRemoveButton(Hub<Section> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected Section");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<Section> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all Sections from the select list");
        return cmd;
    }
    ...
    public SectionJfc getSectionJfc() {
        if (jfcSection == null) {
            jfcSection = new SectionJfc(new Hub<Section>(Section.class));
            OAObjectModel om = jfcSection.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<Section> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getSectionJfc().getHub().setSharedHub(hub, true);
                getSectionJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Section Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                SectionSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!SectionSearchJfc.this.bSelected) {
                    SectionSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getSectionSearch().reset();
    }
    
    protected void onSelect(Section section, Hub<Section> hubFrom) {
        sectionSelected = section;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(section, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<Section> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getSectionSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "Section Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting Sections ... please wait ...");
        dlgWait.setTitle("Section Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No Sections were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            int max = getModel().getSectionSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Sections.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "Section Search",
