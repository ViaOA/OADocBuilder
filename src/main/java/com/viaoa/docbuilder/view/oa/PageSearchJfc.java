...
public class PageSearchJfc {
    private static Logger LOG = Logger.getLogger(PageSearchJfc.class.getName());
    
    private PageSearchModel model;
    private JDialog dialog;
    protected PageJfc jfcPage;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected Page pageSelected;
    protected boolean bSelected;
    
    public PageSearchJfc() {
        this(null, true, false);
    }
    public PageSearchJfc(PageSearchModel model) {
        this(model, true, false);
    }
    public PageSearchJfc(PageSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public PageSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public PageSearchModel getModel() {
        if (model == null) {
            model = new PageSearchModel();
            model.getPageSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("page.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<Page> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Page page = hub.getAO();
                    if (page != null) onSelect(page, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<Page> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("Pages to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for Page
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    Page page = model.getHub().getAO(); 
                    if (page != null) onSelect(page, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                PageJfc jfcPage = new PageJfc(model);
                OATree tree = jfcPage.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getPageSearchHub(), PageSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<Page> hubPage) {
        Hub<Page> hubx = new Hub(Page.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (PageSearchJfc.this.getModel().getMultiSelectHub() != hubPage) {
                    Page page = hubPage.getAO();
                    if (page != null) onSelect(page, hubPage);
                }
            }
        };
        table.setFilterMasterHub(hubPage);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubPage) {
    ...
    protected void createTableColumns(OATable table) {
        getPageJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<Page> hub) {
        ...
        cmd.setToolTipText("Add selected Page");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected Pages");
    ...
    public OAButton createRemoveButton(Hub<Page> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected Page");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<Page> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all Pages from the select list");
        return cmd;
    }
    ...
    public PageJfc getPageJfc() {
        if (jfcPage == null) {
            jfcPage = new PageJfc(new Hub<Page>(Page.class));
            OAObjectModel om = jfcPage.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<Page> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getPageJfc().getHub().setSharedHub(hub, true);
                getPageJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Page Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                PageSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!PageSearchJfc.this.bSelected) {
                    PageSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getPageSearch().reset();
    }
    
    protected void onSelect(Page page, Hub<Page> hubFrom) {
        pageSelected = page;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(page, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<Page> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getPageSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "Page Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting Pages ... please wait ...");
        dlgWait.setTitle("Page Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No Pages were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            int max = getModel().getPageSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Pages.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "Page Search",
