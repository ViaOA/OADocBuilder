...
import com.viaoa.docbuilder.model.oa.Code;
...
public class CodeJfcBase implements OAModelJfcInterface {
    protected static Logger LOG = Logger.getLogger(CodeJfc.class.getName());
    
    public static final String PP_Display    = OAString.cpp(Code.P_Id);
    ...
    // Search
    protected CodeSearchJfc jfcSearch;
    // hub Search
    protected CodeSearchJfc jfcSearchHub;
    // Add
    protected CodeSearchJfc jfcAdd;
    ...
    public CodeJfcBase(Hub<Code> hubCode) {
        this.model = new CodeModel(hubCode);
        this.model.setForJfc(true);
        OAModelJfcUtil.register(this);
    }
    public CodeJfcBase(CodeModel modelCode) {
        if (modelCode == null) this.model = new CodeModel();
        else this.model = modelCode;
        this.model.setForJfc(true);
        OAModelJfcUtil.register(this);
    }
    ...
    public static Icon getIcon() {
        return Resource.getJarIcon("code.gif");
    }
    public static Icon getTitleIcon() {
        return Resource.getJarIcon("codes.gif");
    }
    ...
    public OAMenuItem createNewMenuItem(Hub<Code> hub) {
        if (!getModel().getAllowNew()) return null;
        OAMenuItem mi = new OAMenuItem(hub, OAMenuItem.NEW) {
            @Override
            public void afterActionPerformed() {
                // CodeJfcBase.this.getEditDialog(this).setVisible(true);
                CodeJfcBase.this.onNewCodeCreated();
    ...
    public OAMenuItem createInsertMenuItem(Hub<Code> hub) {
        if (!getModel().getAllowNew()) return null;
    ...
    public OATreeNode createTreeNode(String leadingPropertyPath, Hub<Code> hubRoot, boolean bIncludeChildren, OATreeNode ... parentNodes) {
        if (hubRoot == null && OAString.isEmpty(leadingPropertyPath)) {
            hubRoot = getModel().getHub();
        }
        OATreeNode node = new OATreeNode(OAString.cpp(leadingPropertyPath, Code.P_Id), hubRoot, getHub()) {
            @Override
            public void objectSelected(Object obj) {
                super.objectSelected(obj);
                CodeJfcBase.this.onShowEditPanel();
            }
            @Override
            public void onDoubleClick(Object obj, MouseEvent e) {
                CodeJfcBase.this.onDoubleClickTreeNode();
            }
            @Override
    ...
    public OAMenuItem createSearchMenuItem() {
        ...
        OAMenuItem mi = new OAMenuItem(getHub(), getModel().getDisplayName() + " search ...", Resource.getJarIcon("search.gif"), OAButton.SEARCH) {
            ...
            public Object getSearchObject() {
                Code code = CodeJfcBase.this.onSearch();
                return code;
            }
            @Override
            public void afterActionPerformed() {
    ...
    public OALabel createLabel() {
        OALabel lbl = new OALabel(getHub(), OAString.cpp(Code.P_Id), 6) {
            @Override
            public void customizeRenderer(JLabel lbl, Object object, Object value, boolean isSelected, boolean hasFocus, int row, boolean wasChanged, boolean wasMouseOver) {
                CodeJfcBase.this.customizeRenderer(lbl, CodeJfcBase.this.getHub().getAt(row), isSelected, hasFocus, row, wasChanged, wasMouseOver);
            }
            @Override
            public String getToolTipText(Object object, int row, String defaultValue) {
                return CodeJfcBase.this.getToolTipText(CodeJfcBase.this.getHub().getAt(row), row, defaultValue);
            }
        };
    ...
    protected void customizeRenderer(JLabel lbl, Code code, boolean isSelected, boolean hasFocus, int row, boolean wasChanged, boolean wasMouseOver) {
    }
    protected String getToolTipText(Code code, int row, String defaultValue) {
        return defaultValue;
    }
    ...
    public OAComboBox createComboBox() {
        OAComboBox cbo = new OAComboBox(getHub(), OAString.cpp(Code.P_Id), 6) {
            @Override
            public void customizeRenderer(JLabel lbl, Object object, Object value, boolean isSelected, boolean hasFocus, int row, boolean wasChanged, boolean wasMouseOver) {
                CodeJfcBase.this.customizeRenderer(lbl, CodeJfcBase.this.getHub().getAt(row), isSelected, hasFocus, row, wasChanged, wasMouseOver);
            }
    ...
    public OAList createList() {
        OAList lst = new OAList(getHub(), OAString.cpp(Code.P_Id), 12, 6) {
            @Override
            public void customizeRenderer(JLabel label, JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                ...
                CodeJfcBase.this.customizeRenderer(label, CodeJfcBase.this.getHub().getAt(index), isSelected, cellHasFocus, index, false, false);
            }
        };
    ...
    public OATree createSearchTree() {
        ...
        tnode.setIcon(CodeJfc.getTitleIcon());
        tree.add(tnode);
        node = new OATreeNode(CodeJfc.PP_Display, getHub()) {
            @Override
            public void onDoubleClick(Object obj, MouseEvent e) {
                CodeJfcBase.this.onDoubleClickTreeNode();
            }
        ...
        node.setIcon(CodeJfc.getIcon());
        node.setForegroundColorProperty(CodeJfc.PP_ForeColor);
        node.setBackgroundColorProperty(CodeJfc.PP_BackColor);
        node.setIconColorProperty(CodeJfc.PP_IconColor);
        node.setImageProperty(CodeJfc.PP_Icon);
        node.setFontProperty(CodeJfc.PP_Font);
        node.setMaxImageHeight(20);
        node.setMaxImageWidth(32);
        tnode.add(node);
        return tree;
    ...
    protected String getToolTipText(OATable table, int row, int col, String defaultValue) {
        // Code client = (Code) table.getHub().getAt(row);
        // defaultValue = CodeDelegate.getToolTipText(client, true);
        return defaultValue;
    }
    public OATable createTable() {
        Hub<Code> hubTable;
        if (getModel().getAllowTableFilter()) hubTable = new Hub<Code>(Code.class);
        ...
        OATable table = new OATable(hubTable) {
            ...
            public void onDoubleClick() {
                if (CodeJfcBase.this.getModel().getAllowGotoEdit()) {
                    CodeJfcBase.this.getEditDialog(this).setVisible(true);
                }
            }
        };
        ...
        // table.setColumnProperties(ModelDelegate.getProperties(), "Code");
    ...
    public void createTableColumns(OATable table) {
        OALabel lbl;
        OATableColumn tc;
        tc = table.addColumn("Id", 6, new OALabel(getHub(), OAString.cpp(Code.P_Id)));
        if (getModel().getAllowTableFilter()) {
            tc.setFilterComponent(new OATextFieldFilter(Code.P_Id));
        }
    }
    ...
    protected void createReadOnlyTableColumns(OATable table) {
        OALabel lbl;
        OATableColumn tc;
        lbl = new OALabel(getHub(), Code.P_Id, 6);
    ...
    public void updateToolBar(final JToolBar toolBar, final ToolBarOptions tbo) {
        ...
        if (tbo.bLabel) {
            OALabel lbl = new OALabel(getHub(), OAString.cpp(Code.P_Id));
            lbl.setToolTipTextPropertyPath(OAString.cpp(Code.P_Id));
            lbl.setMaxCols(20);
            lbl.setFont(lbl.getFont().deriveFont(Font.ITALIC));
    ...
    public JButton createGotoListButton() {
        ...
        cmd.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                CodeJfcBase.this.showCardPanel(CodeJfcBase.this.CARD_List);
            }
        });
        OAButton.setup(cmd);
        return cmd;
    ...
    public JDialog getEditDialog(Component comp) {
        ...
        final Hub<Code> hub = new Hub<Code>(Code.class);
        hub.setSharedHub(CodeJfcBase.this.getModel().getHub(), true);
        dlgEdit = new JDialog(win, getModel().getDisplayName(), ModalityType.MODELESS) {
            @Override
            public void setVisible(boolean b) {
                super.setVisible(b);
                if (b) hub.setSharedHub(CodeJfcBase.this.getModel().getHub(), true);
        ...
        CodeJfc jfc = new CodeJfc(getModel());
    ...
    public OALabel createIdLabel() {
        OALabel lbl = new OALabel(getHub(), Code.P_Id, 6);
        lbl.setMaximumColumns(8);
        return lbl;
    }
    
    public OADateTimeTextField createCreatedDateTimeTextField() {
        OADateTimeTextField dttxt = new OADateTimeTextField(getHub(), Code.P_Created, 15);
    ...
    public OATextArea createCodeTextArea() {
        OATextArea txta = new OATextArea(getHub(), Code.P_Code, 2, 30);
        txta.setLineWrap(true);
        txta.setWrapStyleWord(true);
        setup(txta);
        return txta;
    }
    public OALabel createCodeLabel() {
        OALabel lbl = new OALabel(getHub(), Code.P_Code, 20);
        return lbl;
    }
    ...
    public PageJfc getPageJfc() {
        ...
        jfcPage = new PageJfc(getModel().getPageModel()) {
            ...
            protected PageSearchJfc getSearchJfc() {
                ...
                CodeJfcBase.this.getModel().getPageSearchModel().getPageSearch().setMaxResults(1000);
                jfcSearch = new PageSearchJfc(CodeJfcBase.this.getModel().getPageSearchModel());
                return jfcSearch;
            }
            @Override
            protected void onNewPageCreated() {
                getEditDialog(CodeJfcBase.this.getCardPanel()).setVisible(true);
            }
            @Override
    ...
    protected void onNewCodeCreated() {
        onShowEditPanel();
    }
    ...
    // Inner Search
    public CodeSearchJfc getSearchHubJfc() {
        if (jfcSearchHub != null) return jfcSearchHub;
        CodeSearchModel modelSearch = new CodeSearchModel();
        modelSearch.setSearchFromHub(getHub());
        jfcSearchHub = new CodeSearchJfc(modelSearch, true, getModel().getAllowMultiSelect()) {
            @Override
    ...
    protected void onHubSearch() {
        ...
        if (getModel().getAllowMultiSelect()) {
            ...
            for (Code code : getSearchHubJfc().getModel().getMultiSelectHub()) {
                if (getHub().contains(code)) {
                    getModel().getMultiSelectHub().add(code);
                }
            }
            getHub().setAO(getModel().getMultiSelectHub().getAt(0));
        }
        else {
            Code code = getSearchHubJfc().getSelected();
            if (code != null) {
                if (getHub().contains(code)) {
                    getHub().setAO(code);
                }
    ...
    // Search
    protected CodeSearchJfc getSearchJfc() {
        if (jfcSearch == null) {
            jfcSearch = new CodeSearchJfc();
        }
        return jfcSearch;
    }
    protected Code onSearch() {
        getSearchJfc().getModel().beforeInput();
        getSearchJfc().getDialog().setVisible(true);
        Code code = null;
        if (getSearchJfc().wasSelected()) code = getSearchJfc().getSelected();
        return code;
    }
    // Add
    protected CodeSearchJfc getAddJfc() {
        if (jfcAdd == null) {
            jfcAdd = new CodeSearchJfc();
        }
        return jfcAdd;
    }
    protected void onAdd() {
        ...
        Code code = getAddJfc().getSelected();
        if (code == null) return;
        getHub().add(code);
        getHub().setAO(code);
    }
    ...
    public static void main(String[] args) {
        ...
        CodeJfc ui = new CodeJfc() {
            @Override
            protected CodeSearchJfc getSearchJfc() {
                if (this.jfcSearch == null) {
                    this.jfcSearch = new CodeSearchJfc(false) {
                        @Override
