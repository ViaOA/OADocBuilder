...
public class Util {
	 ...
	 * @param fileNameTo
	 * @throws Exception
	 *             protected void copy(ProgressDialog dlgProgress, String fileNameFrom, String fileNameTo) throws Exception { if
	 *             (dlgProgress != null) { dlgProgress.setTitle("Copy File"); dlgProgress.setHeading("Copy file " + fileNameFrom + " to " +
	 *             fileNameTo); dlgProgress.setValue(0); } fileNameFrom = OAString.convertFileName(fileNameFrom); File fileFrom = new
	 *             File(fileNameFrom); if (!fileFrom.exists()) throw new Exception("File " + fileNameFrom + " not found"); fileNameTo =
	 *             OAString.convertFileName(fileNameTo); File fileTo = new File(fileNameTo); fileTo.mkdirs(); fileTo.delete(); int max =
	 *             (int) fileFrom.length(); if (dlgProgress != null) { dlgProgress.setMinimum(0); dlgProgress.setValue(0);
	 *             dlgProgress.setMaximum(max); } InputStream is = new FileInputStream(fileFrom); OutputStream os = new
	 *             FileOutputStream(fileTo); int tot = 0; int bufferSize = 1024 * 8; byte[] bs = new byte[bufferSize]; for (int i=0; ;i++) {
	 *             int x = is.read(bs, 0, bufferSize); if (x < 0) break; tot += x; if (dlgProgress != null) {
	 *             dlgProgress.setValue(Math.min(tot,max)); } os.write(bs, 0, x); } is.close(); os.close(); }
	 */
	...
	public static void createNewWebStartShortcuts() {
		...
		try {
			...
			OAFile.copyResourceToFile(com.viaoa.docbuilder.resource.Resource.class, "desktop/icon.ico", s + "/icon.ico");

			// desktop links
			s = System.getProperty("user.home") + "/Desktop/";
			File file = new File(s + "CDI Scheduler.lnk");
			if (file.exists()) {
				OAFile.copyResourceToFile(com.viaoa.docbuilder.resource.Resource.class, "desktop/CDI Scheduler.lnk", s + "CDI Scheduler.lnk");
			}
			// program menu links
			s = System.getProperty("user.home") + "/Start Menu/Programs/";
			OAFile.copyResourceToFile(	com.viaoa.docbuilder.resource.Resource.class, "desktop/CDI Scheduler.lnk",
										s + "CDI Scheduler/CDI Scheduler.lnk");
	...
	/**
	 * HACK! for Java Web Start - since the local downloaded JNLP file does not always exist. Read the desktop link file to find the
	 * filename that see: "http://www.wotsit.org/" for windows lnk file format
	 */
	public static String readWebstartJNLPCacheFileName(String windowsLinkFileName) throws Exception {
		...
		for (int j = 0; j < 2500; j++) {
			...
			if (matchPos > matchMax) {
				...
				if (ch == 0) {
					; // chars are in double bytes, skip the 0
	...
	/**
	 * "Hack" to fix a bug with Java Web Start, where the desktop and start menu Links do not point to a valid jnlp in the jws cache
	 * directory. This reads the Windows actual file for the desktop short cut icon, and will parse it to find out which file is being
	 * called, which should exist in the JWS cache. If it does not exist, it will be created usig the jnlp file from the website.
	 * 
	 * @throws Exception
	 */
	public static boolean fixWindowsLinksForWebStart() throws Exception {
		...
		if (fixWindowsLinkForWebStart(s, url)) {
			b = true;
		}
	...
	private static boolean fixWindowsLinkForWebStart(String linkFileName, URL urlJnlp) throws Exception {
		...
		LOG.warning("linkFileName=" + linkFileName + " does not exist, will create and copy from URL=" + urlJnlp.toString() + ", to file="
				+ jnlpFileName);
