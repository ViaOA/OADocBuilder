...
public class DataSourceController {
	...
	public boolean loadServerRoot() throws Exception {
        ...
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                String msg = "serverRoot.getProjectInfos()";
                try {
                    alExecutorService.add(msg);
                    if (isUsingDatabase()) {
                        select(serverRoot.getProjectInfos(), "", null);
                        serverRoot.getProjectInfos().loadAllData();
                    }
                    else {
                        OAObjectCacheDelegate.setSelectAllHub(serverRoot.getProjectInfos());
                    }
                }
                catch (Exception e) {
                    String s = "DataSourceController error selecting ProjectInfos, exception="+e;
                    alSelectError.add(s);
                    LOG.log(Level.WARNING, s, e);
                }
                finally {
        ...
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                String msg = "serverRoot.getTutorials()";
                try {
                    alExecutorService.add(msg);
                    if (isUsingDatabase()) {
                        select(serverRoot.getTutorials(), "", null);
                        serverRoot.getTutorials().loadAllData();
                    }
                    else {
                        OAObjectCacheDelegate.setSelectAllHub(serverRoot.getTutorials());
                    }
                }
                catch (Exception e) {
                    String s = "DataSourceController error selecting Tutorials, exception="+e;
                    alSelectError.add(s);
                    LOG.log(Level.WARNING, s, e);
                }
                finally {
